...
%     toc
    sprintf('Calculate S_0_a: ')
%     tic
    S_0_a(1,file_counter) = 0.0;
    for c = 1:numIter
        if c > 1
                if x_airfoil(c) >= x_airfoil(c-1)
                    S_0_a(c,file_counter) = S_0_a(c-1,file_counter)+sqrt((x_airfoil(c)-x_airfoil(c-1))^2+...
                    (y_airfoil_calc(c)-y_airfoil_calc(c-1))^2);
...
%     delta_gpu           = gpuArray(delta);
    ...
    parfor l = 1:numIter 
        ...
        s_airfoil_min(l,file_counter) = S_0_a(min_idx,file_counter);
