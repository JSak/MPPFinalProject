    ...
    for c = 1:numIter
        if c > 1
                if x_airfoil(c) >= x_airfoil(c-1)
                    S_0_a(c,file_counter) = S_0_a(c-1,file_counter)+...
                        sqrt((x_airfoil(c)-x_airfoil(c-1))^2+...
                        (y_airfoil_calc(c)-...
                        (y_airfoil_calc(c-1)))^2);
...
%     parfor l = 1:numIter 
    parfor l = 1:numIter 
        ...
        temp_delta = sqrt((x_airfoil_gpu-x_interface(l)).^2 + ...
            (y_airfoil_calc_gpu-y_interface(l)).^2);
